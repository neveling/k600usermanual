...
\psfig{figure=FIG/GOPR0219.ps,width=16cm,angle=0}}
...
magnetic spectrometer designed and built at the Indiana University Cyclotron Facility (IUCF) in Indiana, USA. % \cite{K600Nim_to_be_submitted}.
...
%  this I did for PR283 slides. just needed a latex infrasrtucture somewhere! dont judge, i was desperate
%
%\begin{equation} 
%	\frac{d\sigma}{d\Omega_{\alpha} dE} =  \frac{\sum_i C_i} { N_t N_{\alpha} \Omega_{\alpha} \delta_{live} \epsilon_{k600} \Delta E   \sum_i \epsilon_{abs,i}(E_{\gamma}) W_i}
%\end{equation} 
%
...
\psfig{figure=FIG/k600spectrometer.ps,width=10cm,angle=-90}}
...
\psfig{figure=FIG/k600-magneticelements-PSchwandt.eps,width=15cm,angle=0}}
\centering
...
\psfig{figure=FIG/k600-dipolefacewindings-PSchwandt.eps,width=13cm,angle=0}}
\centering
...
Maximum solid angle $\Delta\theta\Delta\phi$ (msr)  &4.39&4.39&4.39\\% &6.0&6.0&6.0\\
...
%\psfig{figure=/home/neveling/K600/PICS/FOCALPLANE/high-disp-fp-nowindow-3-formanual.ps,width=14cm}}
 \psfig{figure=FIG/high-disp-fp-nowindow-3-formanual.ps,width=14cm}
}
\centering
...
\psfig{figure=FIG/newman-phd-figD-1-cropped.ps,width=14cm,angle=0}
}
\centering
...
\psfig{figure=FIG/newman-phd-figD-2-cropped.ps,width=8cm,angle=0}
}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/ang-err-calc1.eps,width=10.5cm,angle=0}}
\centering
...
%
%If we consider a  more realistic size of the beamspot, with either a width or height of 4mm,
%then the possible range of angles measured with the pepperpot will be
%representative of the size of the holes of the pepperpot in addition to the range of angles now possible due to the different
%incident angles that can be formed between the extended beamspot and the collimator.
%Calculations for a pepperpot collimator hole 4mm in diameter, a beamspot 4mm across, at a distance of 839mm from the target, 
%yield a {\it range of angles} of d$\theta$ = d$\phi$ $\sim$ 0.55$^{\circ}$ (9.5 mrad). 
%For the case of a smaller beamspot, say with a size of 2mm, calculations for a pepperpot collimator hole 4mm in diameter
%at a distance of 839mm from the target, 
%yield d$\theta$ = d$\phi$ $\sim$ 0.41$^{\circ}$ (7.2 mrad). 
%And a really flat beamspot of 1mm overall width/height yield d$\theta$ = d$\phi$ $\sim$ 0.34$^{\circ}$ (6. mrad). 
...
0.069$^{\circ}$ % (this number still open for debate). % RN check p30 of your K600 book 15
...
The fact that the vertical number is smaller than the horizontal number %, coupled to the fact that 0.221 approaches the value of 0.19  
...
%Also, the naive expectation is that for achromatic beamtune the spread of the beam on the HAtanaka mesh is smaller
%than for dispersion matched beam, hence the fact that the horizontal scattering angle uncertainty is smaller than 
%in the case of the faint beam tune. 
...
%Since we determine
%the vertical angle from a vertical position, which means off-focus mode is required, a faint beam measurement
...
%I will try to extract some data from different experiments, looking at size of the beam on
%Hatanaka as well as the spread in angles of the faintbeam as measured in the focal plane, to see if there is a correlation.

%Also one can try to use pepperpot data and take out the effect of the finite size of the pepperpot collimator holes.
%The numbers quoted in the NIM paper for achromatic beamtune does seem to suggest that with a sufficiently small
%beamspot (and I am assuming a small beamspot on the Hatanaka mesh) the angle resolution will be relatively small.
%The difference between the vertical and horizontal resolution must be beamspot size and angle related.
%Will confirm.
...
%size of the beam on the Hatanaka mesh.
...
%This follows from a $\theta_{fp}$ distribution with a resolution $\sigma_{\theta}$=0.193$^{\circ}$ (FiHM = 0.45$^{\circ}$.
%Does this mean that the angle calibration of this experiment is not good?
%Check that
%Checked. Seems like the ThSCAT calibration is out, should scale thscat up by about 22/18
%also the (theta|theta) term for high disp fp is 1.37
%so 0.193/1.37 = 0.141
%and 0.174 * 22/18 = 0.21
%MMM it turns out that the thetaFP value of the run I looked at run 154, is seriously giving bad thFP values
%with a full range of only 4-5 degr. This just means that the z_x1x2 value used in the analysis is wrong.
%so I decide to only trust the ThSCAT values as extracted from the analysis, as that should be mostly ok.



%Considering only first-order ion optics, horizontal scattering angles with the K600 can be accurately determined
%with its focal plane detectors (i.e. there is a linear relationship between the focal plane angle and the
%scattering angle).
%If we assume that the horizontal angle in the focal plane is determined from two position measurements in two X-wire planes, 
%roughly each with a position accuracy of 0.3mm, we assume the 2 X-wireplanes are 300mm apart, 
%and we assume an average angle of 35$^{\circ}$ (i.e. the detectors in the MDFP), then the horizontal focal plane
%angle uncertainty is $\sim$0.06-0.07$^{\circ}$.
%Since the first order horizontal scattering angle matrix element $(\theta \mid \theta)$ $\sim$ -1.90,
%this translates to a scattering angle uncertainty of $\sim$0.035$^{\circ}$.
...
%In conclusion, from now onwards we should not quote the numbers from the NIM paper regarding angular resolution.
%They represent the width of a peak of a pepperpot distribution. They do not represent the angular resolution capability of a 
%K600 measurement.

%The real angle uncertainty of your measurement will depend on whether you use off-focus mode or not, and if you use
%off-focus mode, how strong was the focus mode. 
%If, for example, you are measuring at big angles, you will only need to make horizontal angle cuts in any way and
%can safely claim scattering angle uncertainty of $\sim$0.05$^{\circ}$.
%If you are measuring at small angles and use a mild off-focus mode, then your effective scattering angle uncertainty would be
%on the order of $\sim$0.1$^{\circ}$.
...
Which means %that $\theta_{fp}=  x_{tgt}s_{21}$ and consequently
\begin{equation} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/ang-err-calc2.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure*}
\centering
\includegraphics[width=0.5\textwidth]{FIG/mc-scat-angle-basic.eps}
...
\begin{figure*}
\centering
\includegraphics[width=0.5\textwidth]{FIG/mc-pepperpot-simple-fit1peak.eps}
...
\begin{figure*}
\centering
\includegraphics[width=0.6\textwidth]{FIG/mc-angle-pos-distri.eps}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{FIG/mc-scat-angle.eps}
\centering
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{FIG/mc-2d-coll-xy.eps}
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{FIG/mc-2d-coll-angle.eps}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{FIG/mc-pepperpot.eps}
\centering
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{FIG/mc-pepperpot-4mmdiam.eps}
\centering
\caption{The intensity distribution of the horizontal component of the real scattering angle for 
...
%Remember that these numbers represent the combined effect of the diameter of the 
%specific hole in the peperpot collimator, the size of the beamspot and angle of the beam
%on the target as well as the inherent resolution of the detection system.
%
%The fact that the vertical number is smaller than the horizontal number %, coupled to the fact that 0.221 approaches the value of 0.19  
%(note that the data for the NIM paper was acquired with the original pepperpot) means that the assumption that most of the uncertainty comes from the beam size on the Hatanaka mesh (or more accurately, the beam emmittance at the object point) is at least plausible, since we know the vertical emmittance to be much bigger than the horizontal beam emmitance.
%
...
%
%For the simulation of the correct finite size of the pepperpot hole (4mm diameter) and with all the beam variations, $\sigma_{\theta_{pp}}$ = 0.127$^{\circ}$. 
To summarize: 
\begin{itemize}
...
\begin{figure}
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/RICHARD/RISHARD1.PS,width=3.5cm}}
\psfig{figure=FIG/RISHARD1.PS,width=3.5cm}}
\centering
...
%\begin{figure}
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/vdc-wireplane-detail.ps,width=10cm,angle=-90}}
%\centering
...
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG//home/neveling/K600/ZEROD/VDC/FIG/VerticalDriftChamber0.ps,width=8.0cm}}
\psfig{figure=FIG/VerticalDriftChamber0.ps,width=8.0cm}}
...
\psfig{figure=FIG/p3240522.ps,width=10cm,angle=0}
...
\begin{figure}
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/VDC/vdc-fieldlines.eps,width=14.0cm}}
\psfig{figure=FIG/vdc-fieldlines.eps,width=14.0cm}}
\centering
...
%\includegraphics[width=5cm]{p9211698.pdf}
 \psfig{figure=FIG/p9121641-small.ps,width=5cm}
 \psfig{figure=FIG/p9121648.ps,width=5cm}
 \psfig{figure=FIG/p9211698.ps,width=5cm}
\end{center}
...
% \psfig{figure=FIG/Richard-PhD-figA2.pdf,width=5cm}
% posres:gaussfit1.*
%\vspace{6.0cm}
...
\shabox{\noindent The Al cover for the focal plane exit window of the K600 is 1.6 mm thick. In the second weekend of PR153 (100 MeV protons) it was seen that 
...
\textrm{ .} 
\end{equation} 
%/home/neveling/K600/PICS/DETECTORS/PADDLES/NEW/
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/ELEC/trig-elec-newdaq.eps,width=17cm,angle=0}}
\psfig{figure=FIG/trig-elec-newdaq.eps,width=17cm,angle=0}
}
\centering
...
\begin{figure}[]
\centerline{\vspace{0cm}\hspace{0cm}
 \psfig{figure=FIG/preamp-logbook-pr128-p11.ps,width=14cm,angle=0}
}
\centering
...
\begin{figure}[]
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/ELEC/camac-vme-cabling-forManual.ps,width=14cm,angle=0}}
 \psfig{figure=FIG/camac-vme-cabling-forManual.ps,width=14cm,angle=0}}
\centering
...
\begin{figure}
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/res2d-goodlut-newdaq.eps,width=10cm,angle=0}}
\psfig{figure=FIG/trigger-timeline.eps,width=15cm,angle=0}}
\centering
...
\centering
%\psfig{figure=FIG//home/neveling/K600/PR153/FIG/hX1res2diTL_3he_chain-120offset.eps,width=7cm,angle=0}
...
%\begin{minipage}{1.9in}
%\centering
%\psfig{figure=FIG/run4095-U1-res2D-ITL-bad.eps,width=7cm,angle=0}
%\end{minipage}
%&
%\begin{minipage}{1.9in}
%\centering
%\psfig{figure=FIG/run4095-U1-res2D-ITL-good.eps,width=7cm,angle=0}
%\end{minipage}
%\end{tabular}
...
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X2res1VSres0-LUTplus60-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X1res1VSres0-LUTplus40-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/U1res1VSres0-LUTplus60-run22295.eps,width=6cm,angle=0}
\end{minipage}
\\   %--------------------------------------------------------------------------------
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X2res1VSres0-LUTplus40-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X1res1VSres0-LUTplus20-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/U1res1VSres0-LUTplus40-run22295.eps,width=6cm,angle=0}
\end{minipage}
\\   %--------------------------------------------------------------------------------
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X2res1VSres0-LUTplus20-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X1res1VSres0-LUT0-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/U1res1VSres0-LUTplus20-run22295.eps,width=6cm,angle=0}
\end{minipage}
\\   %--------------------------------------------------------------------------------
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X2res1VSres0-LUT0-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
%\psfig{figure=/home/neveling/K600/SOFTWARE/MIDAS/k600/PR166/FIG/X1res1VSres0-LUTmin20-run22295.eps,width=6cm,angle=0}
\psfig{figure=FIG/X1res1VSres0-LUTmin20-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/U1res1VSres0-LUT0-run22295.eps,width=6cm,angle=0}
\end{minipage}
\\   %--------------------------------------------------------------------------------
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X2res1VSres0-LUTmin20-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/X1res1VSres0-LUTmin40-run22295.eps,width=6cm,angle=0}
\end{minipage}
&
\begin{minipage}{1.9in}
\centering
\psfig{figure=FIG/U1res1VSres0-LUTmin20-run22295.eps,width=6cm,angle=0}
\end{minipage}
   %--------------------------------------------------------------------------------
\end{tabular}
...
\begin{figure}
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/res2d-badlut-newdaq.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/res2d-bad2lut-newdaq.eps,width=10cm,angle=0}}
\centering
\caption{Driftlength and resolution plots for a good LUT but in need of a 
...
\begin{figure}[hb!]
\centering
\psfig{figure=FIG/relative-vdc-positions-Jun2020.eps,width=16cm,angle=0}
...
%-----------------------------------------------------------
\subsection{Rigidity calibration }\label{sec:rigiditycalibration}   %(by L. Pellegri)
...
\begin{figure}[b!]
	\centerline{\vspace{0cm}\hspace{0cm}
	\psfig{figure=FIG/QBrho-24Mgcalib.eps,width=10cm,angle=0}}
	\centering
...
\begin{figure}
	\centerline{\vspace{0cm}\hspace{0cm}
	\psfig{figure=FIG/run02464-rigidity3.eps,width=10cm,angle=0}}
...
\begin{figure}
	\centerline{\vspace{0cm}\hspace{0cm}
	\psfig{figure=FIG/run02464-Ex-allgates.eps,width=10cm,angle=0}}
...
\begin{figure}
	\centerline{\vspace{0cm}\hspace{0cm}
	\psfig{figure=FIG/scattering.ps,width=10cm,angle=0}}
...
\begin{figure}[]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/x1pos-pid-pr236-run1104.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/thscatvsx1-pr236-run1104.eps,width=10cm,angle=0}}
\centering
...
\shabox{\noindent  Please Note! From time to time modifications to the source code
are implemented to calculate $X1posC$ using $Y2$ instead of $Y1$ for corrections, 
or using a different angle parameter, or calculating the angle parameter in a different manner.
Please make sure what is the status in the version of the code you have, and ensure that
it actually does what you think it does, i.e. PLEASE LOOK IN THE CODE AND VERIFY.}
\bigskip
...
%Consider these as a kind of analysis figures of merit
...
\begin{figure}[]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/CAKE-PID-combined.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/spectrometer-fields-small.ps,width=10cm,angle=0}}
 \psfig{figure=FIG/spectrometer-fields-small.ps,width=10cm,angle=0}}
\centering
...
\psfig{figure=FIG/hysteresisloop.eps,width=16cm,angle=0}}
\centering
...
\psfig{figure=FIG/K600-overshootcurrent.eps,width=16cm,angle=0}}
\centering
...
\psfig{figure=FIG/D1-setfield-bw.ps,width=14cm,angle=180}}
\centering
...
%\psfig{figure=MAGNETS/D2-setfield-bw.ps,width=14cm,angle=0}}
 \psfig{figure=FIG/D2-setfield-bw.ps,width=14cm,angle=0}}
\centering
%\includegraphics[width=\textwidth]{spectrometer-fields-small.pdf}
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/k0-h30-color.ps,width=5.5cm,angle=0}}
%\centering
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/k30-h30-color.ps,width=5.5cm,angle=0}}
%\centering
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/k15-h30-color.ps,width=5.5cm,angle=0}}
%\centering
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/k15-h40-color.ps,width=5.5cm,angle=0}}
%\centering
...
\shabox{\noindent 
The blank collimator should always be in position nr.5, 
since it forms an integral part of the 0$^{\circ}$ interlock 
(refer to section \ref{sec:zero-interlock}).
VERY IMPORTANT: AS OF 1 OCTOBER 2018 WHEN WE MOVED TO THE NEW COLLIMATOR LADDER MECHANISM
WE DO NOT YET HAVE A SIMILAR INTERLOCK CAPABILITY FOR THE SOLID COLLIMATOR
}
\bigskip
...
\psfig{figure=FIG/inserts-1.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/inserts-2.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/49mm1mm-insert-tapering.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/big-63mm-coll-with-49mm11mminsert.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/big-63mm-coll-with-63mm7mminsert.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/63mm-coll-with-55mm7mminsert.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/solid49mm.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/14mm-hole-incoll.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/slot-55-14-in63mmcoll.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/multihole.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/collimator-solid.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/collimator-solid-plug.ps,width=10.5cm,angle=0}
}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/collimator-pepperpot.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/5532-0053.ps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/5562-0051-Pepperpot-insert.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/5562-2600-A-11mm-Collimator.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/5562-2500-A-14mm-Collimator.eps,width=10.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/DSC-0646-dimensions-collimator-BaGeLcampaign.eps,width=10.5cm,angle=0}}
\centering
\caption{A schematic of the collimator setup with the 14mm insert, specific for the collimator ladder setup
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/k600-old-scatchamber.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/K600-oldscat-p2p.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/P4145516.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/2015-05-15-scatchamber.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/scat-cham-exit-original.eps,width=15cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/scat-cham-exit-nolanchamber.eps,width=15cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/scat-cham-exit-new.eps,width=15cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/scat-cham-entry-new.eps,width=15cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/k600-internalbeamstop-pr64b-4-small.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/internal-beamstop-64x64x-slidingseal-opening-1.eps,width=10cm,angle=0}}
\centering
\caption{The internal beamstop for angles $5^{\circ}<\theta_{K600}<21^{\circ}$
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/small-angle-beamstop.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/small-angle-beamstop-frontview.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/new-smallangle-mode-design.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/new-smallanglemode-beamview.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/zerod-beamline-and-beamstop.ps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/ptbeamstop.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\epsfig{figure=FIG/pt-beamstop.eps,width=14cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/pr170-ptbeamstop-P1074185.eps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/tmp2-k600-move.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/k600-smallest-angle-with-beamline-specldwg.ps,width=13cm,angle=0}}
\centering
...
%What worries me more is that maybe a different beam was extracted from
%the SSC after this energy measurement. If a different turn of the SSC
%was extracted then we are of course completely in the dark.
%I had a quick look through the logbook for clues, but have not found any
%yet.  May be you can also see if you can find any mention of
%a) new energy measurements
%b) a major SSC retune.
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\epsfig{figure=FIG/tgt-ladder-standard.ps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\epsfig{figure=FIG/0degr-tgt-ladder-2.eps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\epsfig{figure=FIG/typical-tgts.ps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\epsfig{figure=FIG/tgtladder-angle.eps,width=14cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/bergoz-blm.ps,width=7cm,angle=0}}
\centering
\caption{The BLM}
\label{fig:hatanaka}
\end{figure} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/hatanaka-scint-1.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/p8220020.ps,width=10cm,angle=0}}
\centering
\caption{The so-called Hatanaka mesh/viewer. It was designed so that viewed from 45$^{\circ}$
(as it is used in the beamline) the small blocks are 1mm by 1mm. 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/new-fpbs-2.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/fpbs.eps,width=15cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/new9Xslit.eps,width=11cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/k600-gaspanel.ps,width=13cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/oxysorb-hydrosorb.ps,width=13cm,angle=0}}
\centering
...
in TRANSPORT notation as $b_{16}=0$ and  $b_{26}=0$  \cite{Fuj02} % p20 right column 11 lines from bottom
...
changed after achieving dispersion matching, then you should at least verify that 
the best possible dispersion matching conditions still prevails (by changing Q6 and Q5).}
...
During the November 2008 66 MeV beamtime of PR146 we achieved a dispersion matched condition 
with energy resolution of about 20 keV (see p133 of the logbook). 
After dispersion matching was achieved it was decided that additional kinematic
correction is needed. Some K-coil and H-coil changes were made (see p137 of the logbook). 
Following this a quick scan of Q6S was performed, but we could not regain the 
20 keV energy resolution. At best we could achieve 26 keV.
...
20 keV resoltion.
{\bf This experienced proved the point: do not mess with the K and H coil after dispersion matching. } 
}
...
than 200 MeV (for protons). We e.g. achieved some success with the 100 MeV proton beam of PR137. 
}
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/15oct04-achr-beamspot-smallfile.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/2009_07_04_12_29_19_312_C10_20090704acroSTarget.ps ,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/15oct04-disp-beamspot-1.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/ang-dispmatch-sline-quad.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/Resolution-66-MeV-1.eps,width=18cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/Resolution-66-MeV-2.eps,width=18cm,angle=0}}
\centering
\caption{October 2002, 66 MeV beam.
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/bestresolution.ps,width=14cm,angle=0}}
\centering
...
And since we HAVE to use slit 1P, that also means we have to use cleanup slits later in the beamline.
}
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/QHLine_bw.ps,width=14cm,angle=0}}
\centering
...
%\begin{figure}[!h]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/xvscat-run127-buf-1-200.ps,width=10cm,angle=0}}
%\centering
...
%\begin{figure}[!h]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/xvscat-run127-buf-400-420-24keV.ps,width=16cm,angle=0}}
%\centering
...
%\begin{figure}[!h]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG//home/neveling/RCNP/e282e316/FIG/gyph-gycphc-c12.eps,width=14cm,angle=0}}
%\centering
%\caption{ }
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/PR183-overfocusmode.eps,width=14cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/PR183-underfocusmode.eps,width=14cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/4underfocus-pr183-Xsensitivity.eps,width=14cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/ang-disp-matching.eps,width=5cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/run2080-overfocus-15-YvsTh.eps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/pepperpot-run21336.eps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/0degr-beamlineOLD-detector-setup.eps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/zerodeg-new-hidispplane.eps,width=12cm,angle=0}}
\centering
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/pr101-1stview-0degrbeamviewer.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/collimator-control-new.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/magnets-control-new.ps,width=10cm,angle=0}}
\centering
...
\psfig{figure=FIG/memo_30aug08-beamoptics-RCNP-vs-iTL-page1.ps,width=20cm,angle=0}}
\centering
%\caption{}
%\label{fig:}
\end{figure} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/memo_30aug08-beamoptics-RCNP-vs-iTL-page2.ps,width=20cm,angle=0}}
\centering
%\caption{}
%\label{fig:}
\end{figure} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/memo_30aug08-beamoptics-RCNP-vs-iTL-page3.ps,width=20cm,angle=0}}
\centering
%\caption{}
%\label{fig:}
\end{figure} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/transport-BeamEnvelope-9XtoTGT-dispersive.ps,width=10cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/transport-BeamEnvelope-9XtoTGT-achromatic.ps,width=10cm,angle=0}}
\centering
\caption{Beam envelope from slit 9X to the S-line target for the beam set up dispersively.}
\label{fig:beam-achr}
\end{figure} 
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/INTERNAL-AUTOTRIM.PS,width=7.5cm,angle=90}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/INTERNAL-AUTOTRIM-ZOOMED.PS,width=7.5cm,angle=90}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/autotrim-fig1.ps,width=7.5cm,angle=90}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/autotrim-fig2.ps,width=7.5cm,angle=90}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/paddles-circuitd.eps,width=10.5cm,angle=0}}
\centering
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/dataroom-pic1.ps,width=7.5cm,angle=0}}
%\centering
...
%\begin{figure}[!ht]
%\centerline{\vspace{0cm}\hspace{0cm}
%\psfig{figure=FIG/dataroom-pic2.ps,width=7.5cm,angle=0}}
%\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/res2d-needlutshift-v1.ps,width=5.5cm,angle=0}}
\centering
...
\begin{figure}[!ht]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/res2d.ps,width=5.5cm,angle=0}}
\centering
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_mount_parts.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_mount_parts_assembled.eps}
 \caption{The assembled version of the parts required for mounting CAKE}
 \label{fig:CAKE_mounting_stuff_assembled}
\end{figure}

\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_mount_parts_assembled2.eps}
 \caption{Another view of the assembled CAKE mount.}
 \label{fig:CAKE_mounting_stuff_assembled}
\end{figure}

\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_mount_parts_spacers.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_Manual.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/CAKE_cable.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/20160809_150008.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/20160809_145956.eps}
...
\begin{figure}
\centering
 \includegraphics[width=0.75\textwidth]{FIG/MMM_rings_LEMO_feedthrough.eps}
...
\begin{figure}
 \centering
 \includegraphics[width=0.75\textwidth]{FIG/MMM_sectors_LEMO_feedthrough.eps}
...
\begin{figure}
 \includegraphics[width=\textwidth]{FIG/20160809_150029.eps}
...
\bibitem[Nev11]{Nev11}   R.~Neveling {\it et al.}, Nuclear Instruments and Methods in Physics Research A 654 (2011) 29–39
...
\begin{figure}[!h]
\centerline{\vspace{0cm}\hspace{0cm}
\psfig{figure=FIG/TRACK_Q-chamber.eps ,width=10cm,angle=90}}
\centering
...
%------------------------------------------------------------------
\chapter{Analysis of PR259 (Daniel's notes)}\label{chap:analysisPR259}
...
\includegraphics[height=4 cm]{3-tofVSX1posCoordinatesFit}  
 \end{center}
\vspace*{-4mm}
...
\begin{figure}[h]
 \begin{center}
\includegraphics[height=4 cm]{4-tofVSX1posCorrected}  
 \end{center}
\vspace*{-4mm}
...
\begin{figure}[h]
 \begin{center}
\includegraphics[height=4 cm]{4-tofVSX1posCorrectedZoom}  
 \end{center}
\vspace*{-4mm}
...
What is shown here is how to do the calibration step by step
outside the analyzer,with only knowledge of X1pos in the TTree.
}
\bigskip
